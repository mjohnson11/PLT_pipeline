...
Insights
Settings
VTn_pipeline/scripts/simple_s_estimation_v2.py /
@mjohnson11
mjohnson11 close to ready for submission
Latest commit 575d938 on Dec 10, 2021
 History
 1 contributor
227 lines (213 sloc)  15.8 KB
...
def calc_interval_logslope(row, assay, t1, t2, sum1, sum2, read_cutoff, gens_per_day):
    # simply calculates the slope of log(freq) if boths tps have >=read_cutoff reads (else returns nan)
    if row[assay + '_T' + str(t1)] < read_cutoff or row[assay + '_T' + str(t2)] < read_cutoff:
        return np.nan
    else:
        logdif = np.log(row[assay + '_T' + str(t2)]/sum2) - np.log(row[assay + '_T' + str(t1)]/sum1)
        return logdif / (gens_per_day*(t2-t1))

def get_s_rec(row, assay, tps, read_cutoff):
    # returns a list of s values from different time intervals for this barcode
    use_tps = [i for i in tps if row[assay + '_T' + str(i)] >= read_cutoff]
    return [row['scaled.s_' + str(use_tps[i]) + '_' + str(use_tps[i+1])] for i in range(len(use_tps)-1)]

def simple_s_measuring(td, assay, tps, neut_edges, neut_reference_read_cutoff, gens_per_day, read_cutoff=10):
    ...
    td['Total.Reads'] = np.sum(td[[assay + '_T' + str(t) for t in tps]], axis=1)
    for i in range(len(tps)-1):
        for j in range(i+1, len(tps)):
            s1 = sum(td[assay + '_T' + str(tps[i])])
            s2 = sum(td[assay + '_T' + str(tps[j])])
            td['s_' + str(tps[i]) + '_' + str(tps[j])] = td.apply(lambda r: calc_interval_logslope(r, assay, tps[i], tps[j], s1, s2, read_cutoff, gens_per_day), axis=1)
...
def reduce_bcs(td, tp_names, bc_target, bc_col='BC'):
    """
    ...
    """
    td_shuf = td.sort_values(by=bc_col)  # sorts rows alphabetically, so they are random with respect to total counts
...
def add_ll_column(td, tps, tp_names, s_col, ll_col_name, gens_per_day, bc_col='BC', near_median_s_buffer=0.01):
    ...
    for e in td_edge_grouped:
        ...
        if num_bcs > 2:  # There must be at least 3 bcs to even try this exclusion mehod
            ...
            if len(near_median) > 0:
                e_outside = e[1].loc[~e[1][bc_col].isin(near_median[bc_col])]
                median_rec = near_median[tp_names].sum().as_matrix(tp_names)
                for entry in e_outside.as_matrix([bc_col] + tp_names):
                    bc_to_ll_dif[entry[0]] = outlier_ll_test(np.array([list(median_rec), list(entry[1:])]), [i*gens_per_day for i in tps])
    td[ll_col_name] = td.apply(lambda row: bc_to_ll_dif.setdefault(row[bc_col], 0), axis=1)


# Main running function
def s_estimation(segs, rep_info, output_base, input_base, experiment, ll_cutoff, bc_target_num, neut_edges, exclusion_file=None, neut_ref_read_cutoff=30, tp_read_count_cutoff=5000, assay_to_first_tp=None, bc_out_extra_cols=[], consider_all_edges_neutral=False, exclude_V2Tn_xcontam=False, gens_per_day=10):
    if exclusion_file:
    ...
    for seg in segs:
        ...
        for rep in rep_info[seg]:
            ...
            tp_names = [tp for tp in [assay + '_T' + str(t) for t in range(ftp, 5)] if np.sum(d_tmp[tp]) > tp_read_count_cutoff]
            ...
            tps = [int(tp.split('_T')[-1]) for tp in tp_names]
            ...
            if len(neut_data) > 4 and len(tps) > 2:
                if rep_count == 1 and seg.split('_')[0] in ['G2640', 'G5150', 'G7530', 'G10150'] and len(tps) == 5 and exclude_V2Tn_xcontam:
                    t2_log = np.log(d[tp_names[2]]/np.sum(d[tp_names[2]]))
                    t1_log = np.log(d[tp_names[1]]/np.sum(d[tp_names[1]]))
                    t3_log = np.log(d[tp_names[3]]/np.sum(d[tp_names[3]]))
                    d = d[~((t2_log > t1_log+0.5) & (t2_log > t3_log+0.5))]
                simple_s_measuring(d, assay, tps, neut_edges, neut_ref_read_cutoff, gens_per_day)
                add_ll_column(d, tps, tp_names, 's', 'LL.ratio', gens_per_day)
                ...
                simple_s_measuring(d_cut, assay, tps, neut_edges, neut_ref_read_cutoff, gens_per_day)
                ...
                simple_s_measuring(d_edge, assay, tps, neut_edges, neut_ref_read_cutoff, gens_per_day)
